...
class ConcurrentStateCoordinator:
    ...
    def heal_partition(self, key: str, *, now: datetime) -> list[DeliveryReceipt]:
        ...
        for intent in pending:
            results.append(self.deliver(intent, now=now))
        return results
    ...
    def deliver(
        ...
        *,
        ...
        with self._lock:
            existing_delivery_intent = self._delivery_to_intent.get(intent.delivery_id)
            if existing_delivery_intent is not None:
                prior = self._intents[existing_delivery_intent]
                if prior != intent:
                ...
                committed = self._receipts.get(intent.delivery_id)
                if committed is not None:
                    if committed.state == DeliveryState.INTERRUPTED:
                        return self.recover(intent.delivery_id, now=now)
                    return self._duplicate_receipt(intent, committed, now=now)
            ...
            if intent.key in self._partitioned_keys:
                self._deferred.setdefault(intent.key, []).append(intent)
    ...
    def recover(self, delivery_id: str, *, now: datetime) -> DeliveryReceipt:
        with self._lock:
            ...
            if (
                ...
                self._receipts[delivery_id] = receipt
    ...
    def reconstruct(self, key: str) -> RecoveryReport:
        with self._lock:
            ...
            unexplained = [
                item.intent_id
                for item in intents
                if item.delivery_id not in self._receipts
            ]
